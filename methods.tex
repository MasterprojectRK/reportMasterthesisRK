...
First, the basic network setup based on the well-known \emph{pix2pix} cGAN architecture \cite{Isola2017} and the training process
...
These results were deemed to offer the best comparability to the cGAN- and DNN approaches of this thesis.
...
Next, the cooler matrices were coarsened to binsize \SI{25}{\kilo\bp} to allow comparisons with the cGAN- and DNN results using cooler coarsen.
...
using (arbitariliy selected) chromosome 10 for validation. Due to the small number of training samples for in this setting, windowsize was set to $w=64$ and batch size to $bs=2$.
The rest of the training process remained the same, cf. \cref{sec:methods:cGAN_initial}.
Additionally, HiC-Reg was compared to a cGAN-CNN model with windowsize $w=256$ and CNN embedding, trained on the typical training data set, 
...
-- and TAD detection was used as a measure of prediction quality in the paper, among others.
