...
Several implementations of the original pix2pix network exist on the internet for example the one by \xxx. 
However, certain adaptations needed to be made for the thesis at hand, see \cref{fig:methods:GAN_arch:generator} and \ref{fig:methods:GAN_arch:discriminator}.
First, certain layers in the generator...
\begin{figure}[p]
    \tiny
...
\begin{enumerate}
 \item basic setup pix2pix like C512-C512-...
 \item of course with image
 \item which layers are left out for 64, 128
 \item the number of trainable parameters
 \item the symmetrization process, how and where
 \item activation sigmoid vs. tanh, check again. sigmoid because matrices contain only 0-1 values.
 \item support only powers of two gt. 64 as w 
\end{enumerate}
