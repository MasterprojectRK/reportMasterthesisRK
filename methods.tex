...
\begin{tabular}{lrr}
\hline
\multirow{2}{*}{\textbf{windowsize}} & \multicolumn{2}{c}{\textbf{trainable weights}} \\
                                     & Generator            & Discriminator           \\ \hline
...
The number of trainable parameters for the DNN embedding is shown in \cref{tab:methods:embedding_network_params}, rightmost column.
...
For this thesis, the DNN-embedding was used in two ways.
...
contained about 4.2 to 4.3 million trainable weights, see \cref{tab:methods:embedding_network_params}, middle column for details.
...
and to results from Farr\'e et al., who used data from Drosophila Melanogaster embroynal cells.
...
but Pau Farr\'e kindly provided some of the non-public source code used for the paper \cite{Farre2018a},following private communication.
After some minor bugfixes and adaptions to make the code work with python version 3, it was usable to recompute the published results.

To this end, \emph{Hi-C data} from 16-18 hours old Drosophila Melanogaster embryonal cells by Schuettengruber et al. \cite{Schuettengruber2014} 
was downloaded in text format from  the gene expression omnibus, accession GSE61471, for bin size \SI{10}{\kilo\bp}.

Next, 49 \emph{chromatin features} from modEncode ChIP-seq experiments with 14-16 hours old Drosophila Melanogaster embryonal cells \cite{Roy2010}
were obtained from GEO in the *.gff3 file format required by the provided python application, see \cref{tab:appendix:features_farre}.
...
This explains why table \ref{tab:appendix:features_farre} contains 49 instead of the expected 50 entries. 
Looking at the alphabetical sorting in the table, it is assumed that H3k4me2 is the 50th feature missing in the paper.
Note that in constrast to the bigwig files used for the \acrshort{dnn} and \emph{Hi-cGAN} so far, these files contain locations of statistically significant enrichments,
...
which is easier to match with the implementations of DNN and Hi-cGAN, see below. 
This change did not visually impair the results compaired to the published results.
Validation samples were generated by a 80:20 split of the training/validation set, leaving Pau Farrè's implementation unchanged in this respect.
...
Since the gff3 file format is not useful for the DNN and \emph{Hi-cGAN}, raw reads from the modEncode ChIP-seq experiments \cite{Roy2010} 
...
For the comparisons, the \acrshort{dnn} was used mostly in its initial configuration, with window size $w=80$,
but using a larger batchsize of 100.
Additionally, sample flipping was used as described in the paper \cite{Farre2018a} and implemented in the provided code.
Finally, the \acrshort{sgd} optimizer used so far was replaced by the RMSprop optimizer with standard parameters, namely a learning rate of 0.001. 
Predicted matrices were not scaled.
...
The predicted matrices from Pau Farrè's python program for chromosome 3R were written out as numpy arrays by adding a simple one-liner to the provided code. 
...
In terms of matrix plots, it was atttempted to use the same cutout as in the paper \cite{Farre2018a}.
...
which was thus used for the thesis at hand.
...
The results for the comparison between the DNN, \emph{Hi-cGAN} and Farr\'e et al. are shown at the end of section \ref{sec:results:comparison}.
