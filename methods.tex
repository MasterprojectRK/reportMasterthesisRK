...
Test samples were processed through the network and the resulting submatrices -- actually, vectors corresponding to their upper triangular parts -- were subsequently re-assembled into full Hi-C matrices in cooler format.
...
but were instead taken as $w\times w \times 1$ grayscale images with 32-bit floating point value range [0...1].
...
Furthermore, let $tv(x)$ be the total variation of image $x$ and $vgg(x)$ the output of the perception loss network for image $x$.
The goal of the modified network was now to find weights $w^*$ such that
\begin{equation}
 \omega^* = {arg\,min}_\omega (  \lambda_{pix} \frac{1}{N} \sum_{s=0}^N (M_s - T_s)^2 
                                                     + \lambda_{tv} \sum_{s=0}^N tv( M^\prime_s) 
                                                     + \lambda_{percept} \frac{1}{N} \sum_{s=0}^N (vgg(M^\prime_s) - vgg(T^\prime_s))^2 )
\end{equation}
