...
but using a larger batchsize of 100 and the RMSprop optimizer with standard parameters, namely a learning rate of 0.001. 
Training was performed on samples from chromosome 2L, 2R and 3L, and chromosome X was used for validation.

Hi-cGAN was used with CNN embedding at windowsize $w=64$ using a batchsize of 2
In this case, training was performed on samples from chromosome 2L and 2R, while chromosome 3L was used for validation.

All other parameters of DNN and Hi-cGAN were set to the same values as in sections \ref{sec:methods:dnn} and \ref{sec:methods:hicgan}.
...
After some minor bugfixes and adaptions to python version 3, this code was used to widely recompute the published results.
To this end, the same Hi-C matrix as above was dowloaded from GEO under accession GSE61471,
...
and not mean read counts.

Since Farrè et al have reported Pearson correlations \emph{per position}, a custom
python script was employed to compute such correlations also for the cooler matrices resulting from the DNN- and Hi-cGAN approaches \cite[scripts/corr\_per\_pos.py]{Krauth2021b}.
Additionally, the results from Pau Farrè's python scripts were converted to cooler and Pearson correlations \emph{per distance} were computed as normal.

In terms of matrix plots, the same cutout as in the paper \cite{Farre2018a} was used.
Here, the paper seems to contain an error, since the plotted excerpt is not from chromosome 3R, 15 to \SI{20}{\mega\bp}, as stated in the caption \cite[fig.\xxx]{Farre2018a}
but most likely from chromosome 3R, 16.75 to \SI{21.75}{\mega\bp}.
...
with the blue and red color scheme used in the publication, and the trackfile for pygenometracks was setup to obey the minimum and maximum values 
in the given cutout.
