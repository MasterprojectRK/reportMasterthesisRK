...
and the values were normalized to the interval [-1, 1] by dividing through the maximum bin distance in each dataset to allow for comparisons across distances.
...
Frechet inception distance (FID) seem more common than Pearson correlation.
...
so that for example $\mathit{PSNR}(X) \not=\mathit{PSNR}(k\cdot X)$ for $k\in\mathbb{R}^{>1}$, which seems not optimal for a use case
