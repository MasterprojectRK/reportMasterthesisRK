...
\begin{tabular}{lllll}
\hline
\textbf{feature} & \textbf{accession} &  & \textbf{feature} & \textbf{accession} \\ \hline
...
H3K9acS10P       & SRR870033          &  &                  &                    \\ \hline
\end{tabular}
\caption[Chromatin features from D. melanogaster used for the comparison DNN / cGAN / Farre et al. \cite{Farre2018a}]{Chromatin features from D. melanogaster used for the comparison\\DNN / cGAN / Farre et al. \cite{Farre2018a}}
...
All training for \cref{sec:results:cgan} with windowsize $w=256$ was done on machine 2, 
while computations for  \cref{sec:results:cgan} with windowsizes $w \in \{64,128\}$ were done on machine 1.
...
\SI{20}{\giga\byte} of main memory, as in machine 2, were not enough to train the \acrshort{cgan} at windowsize 64 for more than 100 epochs.
...
\begin{figure}[htbp]
    ...
    \caption{Combined loss with parametrization $\lambda_\mathit{MSE}=10^{-5}$, $\lambda_\mathit{VGG}=0.0$, $\lambda_\mathit{TV}=1.0$}
...
\subsubsection{cGAN and DNN trained on D. melanogaster, chr2L and 3L} \label{sec:appendix:cgan_drosophila}
\begin{figure}[h!] %DNN trained on drosophila 2L, 2R 
    \begin{subfigure}{0.45\textwidth}
        \scriptsize
        \resizebox{\textwidth}{!}{
        \import{figures/DNN_drosophila/}{pearson_chr3L.pdf_tex}}
        \caption{chr3L (validation)}
    \end{subfigure} \hfill
    \begin{subfigure}{0.45\textwidth}
        \scriptsize
        \resizebox{\textwidth}{!}{
        \import{figures/DNN_drosophila/}{pearson_chr3R.pdf_tex}}
        \caption{chr3R (test)}
    \end{subfigure}\\[5mm]
    \begin{subfigure}{0.45\textwidth}
        \scriptsize
        \resizebox{\textwidth}{!}{
        \import{figures/DNN_drosophila/}{pearson_chrX.pdf_tex}}
        \caption{chrX (test)}
    \end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}
        \scriptsize
        \resizebox{\textwidth}{!}{
        \import{figures/DNN_drosophila/}{lossOverEpochs.pdf_tex}}
        \caption{learning progress} \label{fig:appendix:DNN_drosophila_progress}
    \end{subfigure}
    \caption{Results\,/\,metrics DNN, $w=80$, trained on D. melanogaster chr2L and 2R}   \label{fig:appendix:DNN_drosophila_pearson}
\end{figure}
...
\begin{subfigure}{\textwidth}
 ...
 \caption{Matrix from DNN, 50 epochs} \label{fig:appendix:farre-vs-ours_dnn-matrix}
\end{subfigure}\\[3mm]
...
\begin{subfigure}{\textwidth}
 ...
 \caption{Reproduced matrix from Farr\'e et al. \cite{Farre2018a}} \label{fig:appendix:farre-vs-ours_farre-matrix_recreated}
\end{subfigure}\\[5mm]
\begin{subfigure}{\textwidth}
...
\end{subfigure}\\[5mm]
\caption{Comparison between matrix plots from DNN / Hi-cGAN / Farr\'e et al. \cite{Farre2018a} using red/blue color maps}
\end{figure}
