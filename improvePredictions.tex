...
\begin{align}
 ...
 L_d(\mathbf{l, k}) &= \frac{1}{bs}\sum^{bs} \frac{1}{n^2}\sum^{n^2}\left( -\log(\mathrm{sigmoid}(\mathbf{k})) \cdot \mathbf{l} - \log(\mathbf{1} - \mathrm{sigmoid}(\mathbf{k})) \cdot (\mathbf{1} - \mathbf{l}) \right) \label{eq:improve:disc_loss1}\\
