...
\end{figure}
The generator has a so-called U-net architecture \xxx Ronneberger,
while the discriminator is implemented as a patchGAN-discriminator \cite{Isola2017}.
The latter splits the images into patches and tries to decide which of them are real and which are fake.
...
Many ways for training a cGAN exists, and achieving a stable, converging training process can be tricky.
For the thesis at hand, we relied on the  extended loss function proposed by Isola et al. \cite{Isola2017}, 
amended by an additional TV loss function $L_\mathit{TV}$ for noise reduction, \cref{eq:improve:cGAN_loss}.
\begin{align}
...
\end{align}
Here, $L_MAE$ is the mean absolute error (L1 error) between the generated output $G(x,z)$ and the true outputs $y$ 
and $l_\mathit{TV}$ is a total variation loss as in \ref{eq:combined_loss_short}.
This loss function was optimized with the Adam optimizer,
alternately training generator and discriminator.
