...
\begin{align}
 L_G(x, y, z) &=\lambda_\mathit{MAE}L_\mathit{MAE}(G(x,z), y) + \lambda_\mathit{adv} \; L_d(\mathbf{1}, D(x, G(x,z)) +  \lambda_\mathit{TV}L_\mathit{TV}(G(x,z)) \label{eq:improve:cGAN_loss}\\
 L_d(l, k) &= \frac{1}{bs}\sum^{bs} \frac{1}{n^2}\sum^{n^2}\left( -\log(\mathrm{sigmoid}(k)) \cdot l - \log(\mathbf{1} - \mathrm{sigmoid}(k)) \cdot (\mathbf{1} - l) \right) \label{eq:improve:disc_loss1}\\
 \mathrm{sigmoid(m)} &= \frac{e^m}{1+e^m} \label{eq:sigmoid}
\end{align}
...
To obtain a scalar loss value, the mean is taken across both batches and patches. 
The sigmoid function is computed individually for all elements in the $(bs,n,n)$-shaped tensors as shown in \cref{eq:sigmoid}, transforming them into value range [0...1].
Finally, individual factors $\lambda$ can be used to balance the influence of the individual loss portions on the joint loss function.

The discriminator was trained as proposed by Isola et al., as already indicated in \cref{fig:improve:cGAN-approach}.
Here, the binary cross entropy loss was optimized with respect to true and fake samples, \cref{eq:improve:disc_loss_total} .
\begin{align}
 L_D = \lambda_D * ( L_d(\mathbf{1}, D(x,y)) + L_d(\mathbf{0}, D(x,G(x, z)) ) \label{eq:improve:disc_loss_total}
\end{align}
...
$\lambda_D$ is a factor proposed by Isola et al. to slow down the learning process of the discriminator compared to the generator.
...
Compared to other cGAN architectures, \emph{pix2pix} is rather simple, but well studied for different applications like label-to-image transfer,
