...
Several GANs already exist to generate images e.g. from text or from labels.
From abstract point of view, similar to the task at hand.
Chromatin features as ``description'' how the Hi-C matrices could look like.
State of the art approaches have on the order of 50 mio parameters \cite{brock2018}.
But these are used for synthesizing images from many different classes, like in imageNet.
For our task, all Hi-C matrices are superficially similar. So try more simple approach.

Pix2Pix rather simple but popular approach for different tasks like image infill, image from labels, image from sketch etc.
Provide citations. 

Describe the basic setup of pix2pix
Pix2Pix using image input, but we have n times 1D. Bad. 2 ideas how to come around,
using DNN like Farre et al, potentially pretrained, or using new CNN. Maybe mention restriction of $2^x$ for image size
\subsubsection{Using a DNN for feature embedding}
\begin{itemize}
...
\subsubsection{Using a CNN for feature embedding}
\begin{itemize}
