...
but larger binsizes of $b_{feat} = b_{mat} = \SI{25}{\kilo\bp}$ were found beneficial for 
the data used throughout this master thesis.
The results are shown in \cref{sec:initialDNNresults}.
...
one needs to be able to compute gradients of the loss function with respect to the network's weights.
This is generally not the case, or at least very difficult to implement in a computationally efficient way,
for all known TAD calling algorithms.

To overcome the limitations, a novel loss function based on TAD insulation scores \cite{Crane2015} was developed.
...
and the expected size of TADs in the matrix must of course be maintained.
...
similar to the way it is computed for the perception loss.
This way, the loss function is well defined, efficiently computable and tensorflow standard functionality can be used to compute gradients with respect to weights.

The idea behind the score-based approach is visualized in \cref{fig:improve:tad_score_loss_function}\,(E) 
-- local minima, i.\,e. bends in the line plot of the score values, \cref{fig:improve:tad_score_loss_function}\,(F),
...
Indeed, some TAD calling algorithms follow the same approach, i\,e. they first compute insulation scores and then use diverse techniques to detect meaningful local minima in the score values \xxx.
However, these techniques usually consider scores from several diamond sizes, leading to computationally involved and thus slow calculations.

For the thesis at hand, again windowsize $w=80$ and diamond sizes $ds={10,15}$ were used at binsizes of \SI{25}{\kilo\bp}.
