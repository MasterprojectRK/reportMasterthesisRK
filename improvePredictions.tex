...
\subsubsection{Modifying binsize and windowsize}\label{sec:improve:binsize_winsize}
...
this time for windowsize $w=64$ and then use transfer learning to provide better starting values for the weights of the embedding network, 
...
has less parameters than a dense embedding and can efficiently be trained along with the rest of the network even at windowsizes up to 256 bins.
...
Compared to the dense approach, the number of trainable parameters in the CNN embedding is widely independent of the windowsize and 
stays within 4.24...4.29 million parameters for the tested windowsizes 64, 128 and 256, cf. \cref{tab:methods:embedding_network_params} (p.~\pageref{tab:methods:embedding_network_params}).
