...
Note that this thesis is using data from human cell lines, cf.~\cref{sec:methods:input_data},
...
Beyond that, the learning curve indicated overfitting, but the resulting test matrices often did not change much which increasing number of epochs, 
...
The red curves in each correlation plot show the correlation between the corresponding training chromosome 
Hi-C data from GM12878 and the target chromosome Hi-C data from K562.
...
The area under the correlation curves was approximately the same for test chromosomes 3 and 5, slightly better for chromosome 10, 
but worse for chromosomes 19 and 21, cf.~\cref{fig:results:basicDNN_pearson} and \ref{fig:results:basicDNN_10k_pearson}.
However, the ability to predict larger structures was lost, and the matrix plots thus did not look better than the ones for bin size \SI{25}{\kilo\bp}, \cref{fig:results:basic10k_matrices}.
...
the matrix plots seemed worse than the initial ones, \ref{fig:results:25k5_matrices}. 
...
Exchanging the mean squared error for a combined loss function consisting of \acrshort{mse}, TV loss and perceptual loss did not improve the results in the chosen setting
according to sections \ref{sec:improve:combined_loss} and \ref{sec:methods:combined_loss}.
The results for optimizing the combined loss function with weighting parameters according to \cref{eq:methods:combined_loss}
...
The results plotted are the best ones obtained by manual tuning of the multiplicative parameters $\lambda$ in \cref{eq:methods:combined_loss}. 
...
In the matrix plots from \cref{fig:results:scoreloss_matrices}, the true- and predicted scores have been added as a second track, replacing the PCA track. 
Indeed, the score curve computed from the true matrices showed local minima at putative TAD boundaries, as set forth in \cref{sec:improve:TAD_loss},
...
The \acrshort{cgan} with \acrshort{dnn}-embedding according to sections \ref{sec:improve:DNN_embedding} and \ref{sec:methods:dnn-embedding} showed interesting results.
...
also allowed for a stable training of the \emph{Hi-cGAN}, whereby the discriminator loss reached stable epoch-average values after 20 to 30 epochs, 
depending on window size, \cref{fig:results:cGAN64_lossEpochs}.
...
Even though the training process was not as stable as for other investigations on \acrshort{cgan}s in this thesis, 
...
did not make for a stable training process, \ref{fig:results:GAN64_mixed_lossEpochs}.
Despite still acceptable Pearson correlations,  \cref{fig:results:GAN64_mixed_pearson}, the predicted matrices were useless, \cref{fig:results:GAN64_mixed_matrices}.
...
Note that the comparisons have not entirely fair so far, because \emph{Hi-cGAN} and \acrshort{dnn} had in this setting been
...
likely due to the small number of samples in this setting, maybe in combination with insufficient number of training epochs. 
...
\begin{subfigure}{\textwidth}
 ...
 \caption{Hi-cGAN with window size $w=256$; plot restricted to first \SI{1}{\mega\bp}} \label{fig:results:zhang-vs-ours_matrices_ours-full-14}
\end{subfigure}\\[5mm]
\begin{subfigure}{\textwidth}
 \centering
 \resizebox{0.92\textwidth}{!}{
 ...
 \caption{Hi-cGAN with window size $w=64$, trained on chr14 only} \label{fig:results:zhang-vs-ours_matrices_ours-only-14}
\end{subfigure}\\[4mm]
...
\begin{subfigure}{\textwidth}
 ...
 \caption{Hi-cGAN with window size $w=256$, chr17, plot restricted to first \SI{1}{\mega\bp}}\label{fig:results:zhang-vs-ours_matrices_ours-full-17}
\end{subfigure}\\[5mm]
\begin{subfigure}{\textwidth}
 \centering
 \resizebox{0.92\textwidth}{!}{
 ...
 \caption{Hi-cGAN with window size $w=64$, trained on chr17 only} \label{fig:results:zhang-vs-ours_matrices_ours-only-17}
\end{subfigure}
...
Next, the \acrshort{dnn} and Hi-cGAN were trained on data from Drosophila Melanogaster embryonic cells
to allow a comparison with the results from Farr\'e et al. \cite{Farre2018a}, cf. \cref{sec:methods:comparison}.
