...
the results of the DNN- and \acrshort{cgan}-based approaches for predicting Hi-C contact matrices will be presented.
...
The \acrshort{cgan} with DNN-embedding showed interesting results.
...
While the Pearson correlations were mostly worse than the ones of the DNN presented above, compare figures \ref{fig:results:DNN64_pearson} and \ref{fig:results:GAN64-dnn_pearson},
...
Using weight transfer from a pre-trained DNN for the embedding networks further stabilized the training process 
...
While the Pearson correlations showed showed slightly better values for the pre-trained DNN-embedding compared to the non-pre-trained, \cref{fig:results:GAN64_pretrain-dnn_pearson},
...
\begin{figure}[p] %cGAN with DNN, no pretraining, windowsize 64, pearson and progress
    ...
    \end{subfigure}
    \caption{results\,/\,metrics \acrshort{cgan}, DNN embedding, no pre-training, $w=64$, test chromosomes}   \label{fig:results:GAN64-dnn_pearson}
\end{figure}
\begin{figure}[p] %cgan with DNN, no pre-training, winsize 64, matrices
    ...
    \end{subfigure}
    \caption{example predictions, \acrshort{cgan}, DNN embedding, no pre-training, $w=64$, 60 epochs} \label{fig:results:cGAN64-dnn_matrices}
\end{figure}
\begin{figure}[p] %cGAN with DNN, pretrained, windowsize 64, pearson and progress
    ...
    \end{subfigure}
    \caption{results\,/\,metrics, \acrshort{cgan}, DNN embedding, pre-trained, $w=64$, test chromosomes}   \label{fig:results:GAN64_pretrain-dnn_pearson}
\end{figure}
\begin{figure}[p] %cgan with DNN, pretrained, winsize 64, matrices
    ...
    \end{subfigure}
    \caption{example predictions, \acrshort{cgan}, DNN embedding, pre-trained, $w=64$, 100 epochs} \label{fig:results:cGAN64_pretrain-dnn_matrices}
\end{figure}


\subsubsection{cGAN with CNN embedding} \label{sec:results:cgan_cnn}
The DNN embedding also allowed for a stable training of the \acrshort{cgan},
...
For windowsize $w=64$, the Pearson correlations showed worse values than the DNN-approach or the \acrshort{cgan} with DNN embedding, \cref{fig:results:cGAN64_pearson}.
...
\begin{figure}[p] %cGAN with CNN, windowsize 64, pearson and progress
    ...
    \end{subfigure}
    \caption{results\,/\,metrics \acrshort{cgan}, CNN embedding, $w=64$, test chromosomes}   \label{fig:results:cGAN64_pearson}
\end{figure}
\begin{figure}[p] %cgan CNN, 64, matrices
    ...
    \end{subfigure}
    \caption{example predictions, \acrshort{cgan}, CNN embedding, $w=64$, 80 epochs}
...
\begin{figure}[p] %cGAN cnn 128, pearson and progress
    ...
    \end{subfigure}
    \caption{results\,/\,metrics \acrshort{cgan}, CNN embedding. $w=128$, test chromosomes}   \label{fig:results:cGAN128_pearson}
\end{figure}
\begin{figure}[p] %cgan CNN, 128, matrices
    ...
    \end{subfigure}
    \caption{example predictions, \acrshort{cgan}, CNN embedding, $w=128$, 90 epochs} \label{fig:results:cGAN128_matrices}
\end{figure}
...
\begin{figure}[p] %cGAN CNN 256, pearson and progress
    ...
    \end{subfigure}
    \caption{results\,/\,metrics \acrshort{cgan}, CNN embedding, $w=256$, test chromosomes}   \label{fig:results:cGAN256_pearson}
\end{figure}
\begin{figure}[p] %cgan CNN, 256k, matrices
    ...
    \end{subfigure}
    \caption{example predictions, \acrshort{cgan}, CNN embedding, $w=256$, 100 epochs}
\end{figure}
\begin{figure}\ContinuedFloat
    ...
    \end{subfigure}
    \caption{example predictions, \acrshort{cgan}, CNN embedding, $w=256$, 100 epochs} \label{fig:results:cGAN256_matrices}
\end{figure}

\subsubsection{cGAN with mixed DNN / CNN embedding} \label{sec:results:cgan-mixed}
Using a \acrshort{cgan} with mixed embedding, i.\,e. DNN-embedding for the generator
and CNN-embedding for the discriminator, did not make for a stable training process, \ref{fig:results:GAN64_mixed_lossEpochs}.
...
Interestingly, pre-training the DNN improved the situation in this mixed DNN/CNN embedding setup. 
While the training process still seemed unstable, \cref{fig:results:GAN64_pretrained_mixed_lossEpochs},
the predicted matrices looked much better, similar to the ones from the \acrshort{cgan} with DNN-embedding (non-pre-trained),
compare figures \ref{fig:results:GAN64_pretrained_mixed_matrices} and \ref{fig:results:cGAN64-dnn_matrices}.
However, the results were still not better than the ones from the CNN embedding alone, cf. \cref{sec:results:cgan_cnn}.
...
\begin{figure}[p] %cgan mixed 64, pretrained, matrices
    ...
    \end{subfigure}
    \caption{example predictions \acrshort{cgan}, mixed embedding, no pre-training, $w=64$, 20 epochs} 
...
\begin{figure}[p] %cGAN mixed 64, PRE-TRAINED, pearson and progress
    ...
    \end{subfigure}
    \caption{results\,/\,metrics \acrshort{cgan}, mixed embedding, DNN pre-trained, $w=64$, test chromosomes}   \label{fig:results:GAN64_pretrained_mixed_pearson}
\end{figure}
\begin{figure}[p] %cgan mixed 64, pretrained, matrices
    \begin{subfigure}{\textwidth}
        \centering
    ...
    \end{subfigure}
    \caption{example predictions \acrshort{cgan}, mixed embedding, DNN pre-trained, $w=64$, 40 epochs} 
...
Additionally, we tried training our own implementation of HiC-Reg from a previous study project \cite{Krauth2020} on the same training data, binsizes and windowsizes as the \acrshort{cgan} model above
