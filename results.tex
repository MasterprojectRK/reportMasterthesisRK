...
The effect was more pronounced for chromosome 17, which indeed is shorter than chromosome 14 and thus provides less training samples.
...
Obviously, the DNN did not perform well in this setting either.
Since it is sharing the network architecture with the approach from Farr\'e et al., 
...
indicating that this quality measure might not be very useful. 
The Pearson correlations per distance were much worse, \cref{fig:appendix:DNN_drosophila_pearson} (page \pageref{fig:appendix:DNN_drosophila_pearson})
and thus somehow matched the very low usefulness of the predicted matrices.

Hi-cGAN, on the other hand, showed reasonable results for the test chromosomes 3R and X.
While an objective comparison with the results from Farr\'e et al. is difficult due to unavailability of raw data,
the predicted matrices from Hi-cGAN seem to be less blurry and look more like real Hi-C matrices for the only, 
maybe because no correction has been applied.
