...
Obviously, the \acrshort{dnn} did not perform well in this setting either, see \cref{fig:results:farre-vs-ours_dnn-matrix}.
However, since the \acrlong{dnn} is sharing the network architecture with the approach from Farr\'e et al., 
this can only mean that the chosen way of data preparation and binning does not work well for this kind of network.
Interestingly, the Pearson correlation per position still came out better than the one reported by Farr\'e et al.,
indicating that this quality measure might not be suitable, \cref{fig:results:farre-vs-ours_pearson_3R}. 
The Pearson correlations \emph{per distance} showed low values for the \acrshort{dnn}, \cref{fig:appendix:DNN_drosophila_pearson} (page \pageref{fig:appendix:DNN_drosophila_pearson})
and thus matched the very low usefulness of the predicted matrices better than the correlations \emph{per position}.

Hi-cGAN, on the other hand, showed reasonable results for the test chromosomes 3R and X, \cref{fig:results:farre-vs-ours_cgan-matrix}.
While an objective comparison with the results from Farr\'e et al. is difficult due to unavailability of raw data,
the predicted matrices from Hi-cGAN seem to be less blurry and look more like real Hi-C matrices,
maybe in part because no (distance-based) correction has been applied.
It should be noted, however, that assessing the quality of matrix plots is difficult with only a short 
cutout from chr3R, 15-\SI{20}{\mega\bp} at hand, which additionally is obviously shown in corrected form
and not with its original values.

\begin{figure}[htbp]
 \centering
  \resizebox{0.65\textwidth}{!}{
  \scriptsize
  \import{figures/GAN_64_drosophila/}{ppp_chr3L_comparison.pdf_tex}}
  \caption{Comparison Pearson correlations DNN / Hi-cGAN / Farr\'e et al. \cite{Farre2018a}}\label{fig:results:farre-vs-ours_pearson_3R}
\end{figure}
\begin{figure}[htbp]
\begin{subfigure}{\textwidth}
 \centering
 \scriptsize
 \import{figures/DNN_drosophila/}{pred01000_chr3R_015-020.pdf_tex}
 \caption{Matrix from DNN, 1000 epochs} \label{fig:results:farre-vs-ours_dnn-matrix}
\end{subfigure}\\[3mm]
\centering{\scriptsize{\cref{fig:results:farre-vs-ours_dnn-matrix}, \ref{fig:results:farre-vs-ours_cgan-matrix}: predicted matrices on top, true matrices from Schuettengruber et al. \cite{Schuettengruber2014} on bottom}}\\[2mm]
\begin{subfigure}{\textwidth}
 \centering
 \scriptsize
 \import{figures/GAN_64_drosophila/}{pred00100_chr3R_015-020.pdf_tex}
 \caption{Matrix from Hi-cGAN, 100 epochs} \label{fig:results:farre-vs-ours_cgan-matrix}
\end{subfigure}\\[5mm]
\begin{subfigure}{\textwidth}
 \centering{\tiny{Image courtesy Farr\'e, Heurteau, Cuvier, Emberly \cite{Farre2018a}, licensed under Creative Commons Attribution 4.0 License \cite{ccby4}\\X-axis not part of original image}}
 \centering
 \scriptsize
 \import{figures/GAN_64_drosophila/}{predFarre_chr3R_015-020.pdf_tex}
 \caption{Matrix from Farr\'e et al. \cite{Farre2018a}} \label{fig:results:farre-vs-ours_farre-matrix}
\end{subfigure}\\[5mm]
\caption{Comparison between matrix plots from DNN / Hi-cGAN / Farr\'e et al. \cite{Farre2018a}}
\end{figure}
